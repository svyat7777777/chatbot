...
class FacebookChannelService {
  ...
  extractInboundEvents(body) {
    if (String(body?.object || '').trim().toLowerCase() !== 'page') {
      return [];
    }
  ...
  async sendMessage(conversation, message) {
    if (!this.isConfigured()) {
      throw new Error('Facebook channel is not configured');
    }
    ...
    if (!recipientId || !text) {
      throw new Error('Facebook outbound message is missing recipient id or text');
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(`Facebook Send API failed: ${body}`);
    }
...
module.exports = { FacebookChannelService };
