        ...
        <main class="sa-main">
          ...
          <section class="sa-section" data-section="workspaces">
            <div class="sa-toolbar">
              ...
              <select class="sa-select" id="planFilter"><option value="">All plans</option><option value="basic">Basic</option><option value="pro">Pro</option><option value="business">Business</option></select>
            ...
            <div class="sa-table-wrap"><table><thead><tr><th>Workspace</th><th>Owner email</th><th>Plan</th><th>Subscription</th><th>Sites</th><th>Users</th><th>Chats</th><th>Included tokens</th><th>Purchased tokens</th><th>Used tokens</th><th>Remaining tokens</th><th>Usage %</th><th>Last activity</th><th>Actions</th></tr></thead><tbody id="workspaceRows"></tbody></table></div>
          </section>
        ...
        <div class="sa-drawer-panel">
          ...
          <div class="sa-card">
            ...
            <div class="sa-list" style="margin-top:12px;">
              ...
              <form class="sa-action-form" id="changePlanForm"><select class="sa-select" name="plan"><option value="basic">Basic</option><option value="pro">Pro</option><option value="business">Business</option></select><span></span><button class="sa-btn" type="submit">Change plan</button></form>
      ...
      <script>
        (function () {
          ...
          function renderWorkspaces(items) {
            ...
            byId('workspaceRows').innerHTML = items.map((w) => '<tr><td><strong>' + esc(w.workspaceName) + '</strong><div class="sa-sub">' + esc(w.workspaceSlug || w.workspaceId) + '</div></td><td>' + esc(w.ownerEmail || '-') + '</td><td><span class="sa-pill">' + esc(w.plan) + '</span></td><td>' + esc(w.subscriptionStatus || '-') + '</td><td>' + fmt.format(w.sitesCount || 0) + '</td><td>' + fmt.format(w.usersCount || 0) + '</td><td>' + fmt.format(w.conversationsCount || 0) + '</td><td>' + fmt.format(w.includedTokensMonthly || 0) + '</td><td>' + fmt.format(w.purchasedTokens || 0) + '</td><td>' + fmt.format(w.usedTokensCurrentPeriod || 0) + '</td><td>' + fmt.format(w.remainingTokens || 0) + '</td><td><span class="sa-pill ' + (w.usagePercent >= 90 ? 'warn' : 'ok') + '">' + esc(w.usagePercent || 0) + '%</span></td><td>' + esc(date(w.lastSeenAt)) + '</td><td><button class="sa-btn light" data-open="' + esc(w.workspaceId) + '">Open</button></td></tr>').join('');
            byId('tokenRows').innerHTML = items.map((w) => '<tr><td><strong>' + esc(w.workspaceName) + '</strong></td><td>' + esc(w.plan) + '</td><td>' + (w.aiEnabled ? '<span class="sa-pill ok">enabled</span>' : '<span class="sa-pill">disabled</span>') + '</td><td>' + fmt.format(w.includedTokensMonthly || 0) + '</td><td>' + fmt.format(w.purchasedTokens || 0) + '</td><td>' + fmt.format(w.usedTokensCurrentPeriod || 0) + '</td><td>' + fmt.format(w.remainingTokens || 0) + '</td><td>' + esc(w.usagePercent || 0) + '%</td></tr>').join('');
          }
          ...
          async function openWorkspace(id) {
            ...
            byId('drawerSub').textContent = (detail.ownerEmail || '-') + ' · ' + detail.plan + ' · ' + detail.subscriptionStatus;
            const rows = [['Owner', detail.ownerEmail || '-'], ['Plan', detail.plan], ['Subscription', detail.subscriptionStatus], ['Sites', detail.sitesCount], ['Users', detail.usersCount], ['Chats', detail.conversationsCount], ['Included tokens', fmt.format(detail.includedTokensMonthly || 0)], ['Purchased tokens', fmt.format(detail.purchasedTokens || 0)], ['Used tokens', fmt.format(detail.usedTokensCurrentPeriod || 0)], ['Remaining tokens', fmt.format(detail.remainingTokens || 0)]];
